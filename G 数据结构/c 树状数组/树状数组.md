...
int dep[mxn]; //d[i] i点的深度
int dfn[mxn]; //时间戳
int sz[mxn]; //sz[i] i子树的节点数
int tree[mxn]; //树状数组
//时间戳的应用场景：把复杂的图 / 树结构，转成线性序列
//时间戳本质上是DFS 遍历的 “顺序编号”，直接反映了节点被首次访问的先后顺序
//这个特性的关键价值在于：同一子树内的所有节点，它们的时间戳会构成一个连续的区间（比如节点 u 的子树对应 [dfn[u], dfn[u]+sz[u]-1]）。
...
void modify(int u, int x){//向上更新父节点
    for(int i = u; i <= n; i += lowbit(i))
     tree[i] += x;
}
int query(int u){//向下查询区间
    int ans = 0;
